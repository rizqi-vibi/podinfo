...
port="9898"
imageUrl="dki-images-registry-vpc.ap-southeast-5.cr.aliyuncs.com/dki/${dkiServiceName}"
if [[ $dkiServiceName == "customer-api" ]]; then
  shortServiceName="idc.cust"
elif [[ $dkiServiceName == "dkisprivyid-api" ]]; then
  shortServiceName="dkisprivyid"
else
  getShortServiceName=$(echo $dkiServiceName | cut -d '-' -f1)
...
function varsReplace {
    sed -i "s|%{SERVICE_NAME}|${dkiServiceName}|g" *.yaml
